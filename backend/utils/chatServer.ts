...
export const chatServer = () => {
    ...
    io.use(async (socket: Socket, next: (err?: ExtendedError | undefined) => void) => {
        const accessToken = socket.handshake.query.accessToken as string;
        jwt.verify(accessToken, process.env.JWT_SECRET!, async (err, token) => {
            if (!err) {
        ...
        const user: LeanDocument<IUser & {_id: Types.ObjectId;}> = socket.data.user;
        ...
        if (!group || !user) {
            return new Error('Cannot find group in range or user');
        }
